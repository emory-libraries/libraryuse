...
(function (Highcharts) {
    ...
    var today = new Date(), 
        monthsAgo = 2,
        default_start = convertDate(new Date(today.getFullYear(), today.getMonth()-monthsAgo, today.getDate())),
        default_end = convertDate(today);

    // Add "Download CSV" to the exporting menu.
    if (Highcharts.getOptions().exporting) {
        Highcharts.getOptions().exporting.buttons.contextButton.menuItems.push({
            text: Highcharts.getOptions().lang.downloadCSV || 'Download Raw CSV',
            onclick: function () {
                var dates = []
                ...
                var link ="/export/"+convertDate(dates[0])+"/"+convertDate(dates[1])+"/";
                var a;
